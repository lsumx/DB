        ...
        try {
            if (is_time_valid()){
                if (can_select(id, course_id)){
                    ...
                    utils.getStatement(sql_insert).setString(1,id);
                    utils.getStatement(sql_insert).setString(2,course_id);
                    utils.getStatement(sql_insert).execute();
                    if (!utils.getStatement(sql_insert).getResultSet().next()){
                        String sql_select ="select * from course_select where s_id =? and course_id =?";
                        utils.getStatement(sql_select).setString(1,id);
                        utils.getStatement(sql_select).setString(2,course_id);
                        utils.getStatement(sql_select).execute();
                        if (utils.getStatement(sql_select).getResultSet().next()){
                            System.out.println("你已经选该门课的选课申请");
                            return false;
                        }else {
                            String sql ="insert into course_select(s_id,course_id,reason) values(?,?,?)";
                            utils.getStatement(sql).setString(1,id);
                            utils.getStatement(sql).setString(2,course_id);
                            utils.getStatement(sql).setString(3,reasons);
